...
import org.apache.spark.scheduler.cluster.CoarseGrainedClusterMessagesLambda._
import org.apache.spark.scheduler.cluster.CoarseGrainedSchedulerBackendLambda.ENDPOINT_NAME
 ...
 */
private[spark]
class CoarseGrainedSchedulerBackendLambda(scheduler: TaskSchedulerImpl, val rpcEnv: RpcEnv)
  extends ExecutorAllocationClient with SchedulerBackend with Logging
{
  ...
  // must be protected by `CoarseGrainedSchedulerBackendLambda.this`. Besides, `executorDataMap` should
  // only be modified in `DriverEndpoint.receive/receiveAndReply` with protection by
  // `CoarseGrainedSchedulerBackendLambda.this`.
  private val executorDataMap = new HashMap[String, ExecutorDataLambda]

  // Number of executors requested from the cluster manager that have not registered yet
  @GuardedBy("CoarseGrainedSchedulerBackendLambda.this")
  ...
  class DriverEndpoint(override val rpcEnv: RpcEnv, sparkProperties: Seq[(String, String)])
    ...
    override def onStart() {
      logDebug("LAMBDA: 7001: CoarseGrainedSchedulerBackendLambda")
    ...
    override def receive: PartialFunction[Any, Unit] = {
      case StatusUpdate(executorId, taskId, state, data) =>
        logDebug("LAMBDA: 7002: CoarseGrainedSchedulerBackendLambda")
      ...
      case KillTask(taskId, executorId, interruptThread) =>
        logDebug("LAMBDA: 7004: CoarseGrainedSchedulerBackendLambda")
        executorDataMap.get(executorId) match {
          case Some(executorInfo) =>
    ...
    private def removeExecutor(executorId: String, reason: ExecutorLossReason): Unit = {
      ...
      executorDataMap.get(executorId) match {
        case Some(executorInfo) =>
          // This must be synchronized because variables mutated
          // in this block are read when requesting executors
          val killed = CoarseGrainedSchedulerBackendLambda.this.synchronized {
    ...
    protected def disableExecutor(executorId: String): Boolean = {
      val shouldDisable = CoarseGrainedSchedulerBackendLambda.this.synchronized {
        if (executorIsAlive(executorId)) {
          executorsPendingLossReason += executorId
          true
        } else {
  ...
  override def start() {
    logDebug("LAMBDA: 7007: CoarseGrainedSchedulerBackendLambda")
  ...
  /**
   * Reset the state of CoarseGrainedSchedulerBackendLambda to the initial state. Currently it will only
  ...
  override def isReady(): Boolean = {
    logDebug("LAMBDA: 7008: CoarseGrainedSchedulerBackendLambda")
  ...
  override def getExecutorIdsLambda(): Seq[String] = {
   ...
   */
  final override def requestExecutorsLambda(numAdditionalExecutors: Int): Boolean = {
    logDebug("LAMBDA: 7009: CoarseGrainedSchedulerBackendLambda")
  ...
  override final def requestTotalExecutorsLambda(
    ...
    logDebug("LAMBDA: 7011: CoarseGrainedSchedulerBackendLambda")
    val response = synchronized {
    ...
    logDebug("LAMBDA: 7012: CoarseGrainedSchedulerBackendLambda")
    val returnValue = defaultAskTimeout.awaitResult(response)
   ...
   *         false.
   */
  final override def killExecutorsLambda(executorIds: Seq[String]): Seq[String] = {
  ...
  //ATA_AMAN: Adding redundant function definitions to b overriden
  //by definitions in other class, in future should combimne both
...
private[spark] object CoarseGrainedSchedulerBackendLambda {
