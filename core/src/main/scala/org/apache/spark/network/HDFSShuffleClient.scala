...
//AMAN: Creating HDFS Shuffle Client
class HDFSShuffleClient(transportConf: TransportConf,
                      blockManager: BlockManager) extends ShuffleClient with Logging {
    
    logInfo("AMAN: Creating HDFSShuffleClient")
    ...
    logInfo(s"AMAN: Fetch blocks from $host:$port (executor id $execId)")
    try {
      val blockFetchStarter = new RetryingBlockFetcher.BlockFetchStarter {
        override def createAndStart(blockIds: Array[String],
                                    listener: BlockFetchingListener): Unit = {
          new HDFSBlockFetcher(blockManager, execId, blockIds, listener).start
        }
      }
