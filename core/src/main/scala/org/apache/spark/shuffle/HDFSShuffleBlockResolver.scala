...
import org.apache.spark.network.buffer.{ManagedBuffer, HDFSSegmentManagedBuffer}
...
import org.apache.spark.util.Utils
  ...
  */
private[spark] class HDFSShuffleBlockResolver(
    conf: SparkConf,
  ...
  val shuffleHDFSNode = BlockManager.getHDFSNode(conf)
  ...
  private def checkIndexAndDataFile(index: File, data: File, blocks: Int): Array[Long] = {
    val indexFilePath = Utils.localFileToHDFS(shuffleHDFSNode, index)
    try {
    ...
    val dataPath = Utils.localFileToHDFS(shuffleHDFSNode, data)
  ...
  private def getFile(localDirs: Array[String], subDirsPerLocalDir: Int, filename: String) = {
    logInfo(s"AMAN: In GET FILE Function, Filename = $filename")
    val hash : Int = Utils.nonNegativeHash(filename)
    logInfo(s"AMAN: GET FILE hash = $hash")
...
private[spark] object HDFSShuffleBlockResolver {
