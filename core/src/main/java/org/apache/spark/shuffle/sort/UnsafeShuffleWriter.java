...
public class UnsafeShuffleWriter<K, V> extends ShuffleWriter<K, V> {
  ...
  private long[] mergeSpills(SpillInfo[] spills, File outputFile) throws IOException {
    ...
    final boolean shuffleOverHDFSEnabled = blockManager.shuffleOverHDFSEnabled();
    long[] partitionLengths;

    if (shuffleOverHDFSEnabled) {
      Path outputPath = Utils.localFileToHDFS(
              blockManager.getHDFSNode(sparkConf), outputFile);
  ...
  private long[] mergeSpillsWithFileStream(
    ...
    if (blockManager.shuffleOverHDFSEnabled()) {
     Path outputPath = Utils.localFileToHDFS(
             blockManager.getHDFSNode(sparkConf), outputFile);
     FileSystem fileSystem = outputPath.getFileSystem(
             BlockManager.getHadoopConf(sparkConf));
