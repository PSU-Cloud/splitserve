...
class TaskSchedulerImplSuite extends SparkFunSuite with LocalSparkContext with BeforeAndAfterEach
  ...
  test("Scheduler does not always schedule tasks on the same workers") {
    ...
    val numFreeCores = 1
    val workerOffers = IndexedSeq(new WorkerOffer("executor0", "host0", numFreeCores, 123456789),
      new WorkerOffer("executor1", "host1", numFreeCores, 123456789))
  ...
  test("Scheduler correctly accounts for multiple CPUs per task") {
    ...
    val zeroCoreWorkerOffers = IndexedSeq(new WorkerOffer("executor0", "host0", 0, 123456789),
      new WorkerOffer("executor1", "host1", 0, 123456789))
    val taskSet = FakeTask.createTaskSet(1)
    taskScheduler.submitTasks(taskSet)
    ...
    val numFreeCores = 1
    val singleCoreWorkerOffers = IndexedSeq(new WorkerOffer("executor0", "host0", numFreeCores, 123456789),
      new WorkerOffer("executor1", "host1", numFreeCores, 123456789))
    taskScheduler.submitTasks(taskSet)
    ...
    // is chosen.
    val multiCoreWorkerOffers = IndexedSeq(new WorkerOffer("executor0", "host0", taskCpus, 123456789),
      new WorkerOffer("executor1", "host1", numFreeCores, 123456789))
    taskScheduler.submitTasks(taskSet)
  ...
  test("tasks are not re-scheduled while executor loss reason is pending") {
    ...
    val e0Offers = IndexedSeq(new WorkerOffer("executor0", "host0", 1, 123456789))
    val e1Offers = IndexedSeq(new WorkerOffer("executor1", "host0", 1, 123456789))
  ...
  test("abort stage if executor loss results in unschedulability from previously failed tasks") {
    ...
    val firstTaskAttempts = taskScheduler.resourceOffers(IndexedSeq(
      new WorkerOffer("executor0", "host0", 1, 123456789),
      new WorkerOffer("executor1", "host1", 1, 123456789)
    ), "VM").flatten
    ...
    val nextTaskAttempts =
      taskScheduler.resourceOffers(IndexedSeq(new WorkerOffer("executor0", "host0", 1, 123456789)), "VM").flatten
    ...
    taskScheduler.resourceOffers(IndexedSeq(new WorkerOffer("executor0", "host0", 1, 123456789)), "VM")
  ...
  test("don't abort if there is an executor available, though it hasn't had scheduled tasks yet") {
    ...
    val offers = IndexedSeq(
      ...
      new WorkerOffer("executor0", "host0", 4, 123456789),
      new WorkerOffer("executor1", "host1", 4, 123456789)
    )
  ...
  test("SPARK-16106 locality levels updated if executor added to existing host") {
    ...
    val secondTaskDescs = taskScheduler.resourceOffers(
      IndexedSeq(new WorkerOffer("executor2", "host0", 1, 123456789)), "VM").flatten
    ...
    val thirdTaskDescs = taskScheduler.resourceOffers(
      IndexedSeq(new WorkerOffer("executor3", "host1", 1, 123456789)), "VM").flatten
  ...
  test("if an executor is lost then the state for its running tasks is cleaned up (SPARK-18553)") {
    ...
    val e0Offers = IndexedSeq(WorkerOffer("executor0", "host0", 1, 123456789))
    val attempt1 = FakeTask.createTaskSet(1)
