...
public class HDFSSegmentManagedBuffer extends ManagedBuffer {
  ...
  public HDFSSegmentManagedBuffer(TransportConf conf, Configuration hadoopConf, Path file, long offset, long length)
          throws IOException {
    this.conf = conf;
    this.file = file;
    this.offset = offset;
  ...
  private FSDataInputStream seekToOffset(long offset) throws IOException {
    ...
    try {
      ...
      // TODO: Check that no double copy happens here. 
      // TODO: Check if we can create these buffers outside heap like tungsten.
